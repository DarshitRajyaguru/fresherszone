...
function fresherszone_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'fresherszone' ),
...
function fresherszone_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'fresherszone' ),
 ...
 * @return bool|string
 */
function fresherszone_the_excerpt( $trim_character_count = 0 ) {
	$post_ID = get_the_ID();
 ...
 */
function fresherszone_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="fresherszone-read-more text-white mt-3 btn btn-info" href="%1$s">%2$s</a>',
			get_permalink( get_the_ID() ),
			__( 'Read more', 'fresherszone' )
		);
	}
